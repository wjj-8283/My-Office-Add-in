        ...
        if (Object.keys(exports.fixedNamingsV3).includes(name)) {
            ...
            switch (type) {
                case FileType.CONFIG:
                    return teamsfx_api_1.ok(`${configPrefix}${res}`);
                case FileType.USERDATA:
                    if (res.startsWith("STATE__"))
                        return teamsfx_api_1.ok(`${secretPrefix}${res.substring(res.indexOf("STATE__") + 7)}`);
                    else
                        return teamsfx_api_1.ok(`${secretPrefix}${res}`);
                case FileType.STATE:
                default:
                    return teamsfx_api_1.ok(res);
            }
...
function commonNamingConverterV3(name) {
    const names = name.split(".");
    return names.join("__").replace(/\-/g, "_").toUpperCase();
}
function provisionOutputNamingConverterV3(name, bicepContent) {
    const names = name.split(".");

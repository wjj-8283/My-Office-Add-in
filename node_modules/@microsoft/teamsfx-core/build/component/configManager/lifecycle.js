...
class Lifecycle {
    ...
    async execute(ctx) {
        ...
        ctx.logProvider.info(`[${constant_1.component}]Executing lifecycle ${this.name}`);
        const resolved = [];
        const unresolved = [];
        ...
        let failedAction;
        if (result.isOk()) {
            ctx.logProvider.info(`[${constant_1.component}]Finished Executing lifecycle ${this.name}. Result: ${Lifecycle.stringifyOutput(result.value)}`);
        }
        else {
            if (result.error.kind === "Failure") {
                e = result.error.error;
                ctx.logProvider.info(`[${constant_1.component}]Failed to Execute lifecycle ${this.name}. ${e.name}:${e.message}`);
            }
                ...
                if (result.error.reason.kind === "DriverError") {
                    ...
                    ctx.logProvider.info(`[${constant_1.component}]Failed to Execute lifecycle ${this.name} due to failed action: ${failedAction}. ${e.name}:${e.message}. Env output: ${output}`);
                }
                    ...
                    ctx.logProvider.info(`[${constant_1.component}]Failed to Execute lifecycle ${this.name} because there are unresolved placeholders ${JSON.stringify(unresolved)} for action: ${failedAction}. Env output: ${output}`);
                }
            }
        }
    ...
    async executeImpl(ctx, resolved, unresolved) {
        ...
        for (const driver of drivers) {
            ctx.logProvider.info(`[${constant_1.component}]Executing action ${this.stringifyDriverDef(driver)} in lifecycle ${this.name}`);
            resolveDriverDef(driver, resolved, unresolved);
            if (unresolved.length > 0) {
                ctx.logProvider.info(`[${constant_1.component}]Unresolved placeholders(${unresolved}) found for Action ${this.stringifyDriverDef(driver)} in lifecycle ${this.name}`);
                summaries.push([
            ...
            ctx.logProvider.info(`[${constant_1.component}]Action ${this.stringifyDriverDef(driver)} in lifecycle ${this.name} succeeded with output ${Lifecycle.stringifyOutput(result.value)}`);
        }
    ...
    resolveDriverInstances(log) {
        var _a;
        log.debug(`[${constant_1.component}]Trying to resolve actions for lifecycle ${this.name}`);
        const drivers = [];
        for (const def of this.driverDefs) {
            ...
            log.debug(`[${constant_1.component}]Action ${this.stringifyDriverDef(def)} found for lifecycle ${this.name}`);
        }

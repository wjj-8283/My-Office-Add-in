import { FolderQuestion, Inputs, OptionItem, QTreeNode, SingleFileQuestion, SingleSelectQuestion, TextInputQuestion } from "@microsoft/teamsfx-api";
export declare const OfficeAddinItems: OptionItem[];
export declare const ImportAddinProjectItem: OptionItem;
export declare const OfficeAddinItem: OptionItem;
export declare enum QuestionName {
    AddinLanguageQuestion = "addin-language",
    AddinNameQuestion = "addin-name",
    AddinProjectFolderQuestion = "addin-project-folder",
    AddinProjectManifestQuestion = "addin-project-manifest",
    AddinTemplateSelectQuestion = "addin-template-select",
    OfficeHostQuestion = "addin-host"
}
export declare const AddinNameQuestion: TextInputQuestion;
export declare const AddinLanguageQuestion: SingleSelectQuestion;
export declare const OfficeHostQuestion: SingleSelectQuestion;
export declare const AddinProjectFolderQuestion: FolderQuestion;
export declare const AddinProjectManifestQuestion: SingleFileQuestion;
export declare const getTemplate: (inputs: Inputs) => string;
export declare const getQuestionsForScaffolding: () => QTreeNode;
//# sourceMappingURL=question.d.ts.map